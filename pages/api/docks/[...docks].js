...
export default nextConnect({
  ...
  .get('api/docks', async (req, res, next) => {
    const pagination = await paginate('dock', serializers, req)
    return res.status(206).json(pagination)
  })
  .get('api/docks/:id', async (req, res) => {
    const id = parseInt(req?.params?.id)

    const selectedFields = formatSerializer('retrieve', serializers)

    const padlock = await prisma.dock.findUnique({
